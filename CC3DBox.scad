...
// Which bottom plate?
profile = 1;		// [0:Standard,1:Low Profile]
...
// Board measures:
boardSize 		= 36;		// Actual size is 36.0
boardThickness	=  1.6;
holeDistance	= 30.5;
holeDiameter	=  3.0;
 ...
 */
module main() {
	translate( [boxSize/2, boxSize/2, bottomLowHeigth/2] ) cc3dLowBottom();
  translate( [boxSize/2, boxSize/2, bottomLowHeigth+topHeigth/2] ) rotate(a=[180,0,0]) cc3dTop();
	translate( [boxSize/2, boxSize/2, bottomLowHeigth+topHeigth+coverHeigth/2] ) cc3dCover(servoHeader = coverPlate);
  echo( "Box height : " );
  echo( bottomLowHeigth+topHeigth+coverHeigth );
  }
